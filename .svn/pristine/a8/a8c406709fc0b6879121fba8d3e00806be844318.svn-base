...
def sprogrammer (master=None):
    oAppSprog = app_qtsprog.app_qtsprog(master)
...
class SProgrammer:
	...
	def vTerminalModeCB(self, event=None):
		E = subprocess.Popen("sudo avrdude -C bin/Modified_avrdude.conf -p ft0 -c usbasp -p AT89S52 -t",shell = True)
		return
	...
	def bOpenIhxFile (self, ihxfile):
		...
		for line in lines[:-1]:
			...
			dicMemMapSorted = sorted(dicMemMap.iteritems(), key=operator.itemgetter(0))
